...
import { BellRing, CalendarClock, Check, Clock, Target } from "lucide-react";
...
export function RelancesManager() {
  ...
  const load = () => {
    ...
    fetch(`/api/leads${q}`)
      .then((r) => r.json())
      .then((p) => {
        if (Array.isArray(p?.data)) setLeads(p.data as Lead[]);
        else setError(p?.error?.message ?? "Chargement des relances impossible.");
      })
